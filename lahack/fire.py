...
doc_ref = db.collection(u'restaurant').document(u'+19293096662')
doc_ref.set({
    u'phone': u'+19293096662',
    u'name': u'In-n-Out',
    u'zipCode': u'90024',
    u'address': u'1100 Gayley Avenue',
    u'messageList': {
    	u'message1': {
    		u'body': u'',
    		u'startTime': u'',
...
def mymessage(call, phoneNumber):
	...
	delete_all_old()
	newcall = call.split()
	if(len(newcall) < 2 or len(newcall) == 3):		# it is either 2 or greater than 3
		return "error 2"
	doc_ref = db.collection(u'restaurant').document(phoneNumber)
	if (len(newcall) == 2):
